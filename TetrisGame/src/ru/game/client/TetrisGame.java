...
public class TetrisGame extends Shell {
	...
	public static void main(String args[]) 
	{
		...
		TetrisGame shell = new TetrisGame(display, SWT.TITLE | SWT.CLOSE | SWT.MIN);// кнопочки
																							// свернуть,
																							// закрыть
		...
		while (!shell.isDisposed()) {
			if (!display.readAndDispatch())// читает события из очереди
											// сообщений
				display.sleep();
		}
		display.dispose();
	...
	protected void createContentsofWindow() {
		...
		tetris.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true)); // заполнение
																			// по
																			// горизонтали
																			// и
																			// вертикали
